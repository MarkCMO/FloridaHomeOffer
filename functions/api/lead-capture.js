...
export async function onRequest(context) {
  ...
  const lead = {
    ...
    state: data.state ? String(data.state).trim().slice(0, 50) : 'FL',
  ...
  // Send emails via Resend (best-effort, don't fail the request)
  try {
    const resend = new Resend(env.RESEND_API_KEY);
    const fromVisitor = env.RESEND_FROM || 'OneCashOffer <onboarding@resend.dev>';
    const fromInternal = env.RESEND_FROM_INTERNAL || 'OneCashOffer Leads <onboarding@resend.dev>';
    ...
    try {
      await resend.emails.send({
        ...
        html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0B1120;color:#E0E4EC;padding:40px;border-radius:8px;">
            <div style="text-align:center;margin-bottom:24px;"><h1 style="color:#C8A84E;font-size:28px;margin:0;">OneCashOffer</h1></div>
        ...
        await resend.emails.send({
          ...
          html: `<div style="font-family:monospace;padding:20px;">
              ...
              <table style="border-collapse:collapse;width:100%;">
                ...
                <tr><td style="padding:6px;font-weight:bold;border-bottom:1px solid #ddd;">Source</td><td style="padding:6px;border-bottom:1px solid #ddd;">${lead.source_page || '-'}</td></tr>
                ...
                <tr><td style="padding:6px;font-weight:bold;border-bottom:1px solid #ddd;">Landing</td><td style="padding:6px;border-bottom:1px solid #ddd;">${lead.landing_page || '-'}</td></tr>
                <tr><td style="padding:6px;font-weight:bold;">Submitted (UTC)</td><td style="padding:6px;">${new Date().toISOString()}</td></tr>
              </table>
            </div>`
        });
